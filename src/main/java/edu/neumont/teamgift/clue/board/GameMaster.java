...
//    }
     ...
     *
     * @param index The index of the player needed.
    ...
    public Player whoHasSuggested(Player whoSuggested, String[] whatSuggested) {
        ...
        for (int i = whoSuggested.getID() + 1; i < numPlayers + whoSuggested.getID();
                i++) {
            ...
            for (int j = 0; j < getPlayerList(whereWeAre).getDeck().size(); j++) {
                for (int k = 0; k < whatSuggested.length; k++) {
                    if (getPlayerList(whereWeAre).getDeck().get(j).getName()
    ...
    public void makeAccusation(Player whoIsAccusing, Dealer d, String weapon,
                               String player, String room) {
        if (isAccusationTrue(d, weapon, player, room)) {
    ...
    public boolean isAccusationTrue(Dealer d, String weapon, String player, String room) {
        return weapon.equals(d.getKillerCaseFile().get(0).getName())
                && player.equals(d.getKillerCaseFile().get(1).getName())
                && room.equals(d.getKillerCaseFile().get(2).getName());
    }
