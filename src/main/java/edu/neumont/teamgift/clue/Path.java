...
class Path {
	 ...
	 *            The destination.
	 * @param distance
	 *            The max distance allowed to travel.
	 */
			...
			for (int j = -1; j <= 1; j++) {
				try {
					...
					boolean blocked = t instanceof Solid
							|| (t instanceof PlayerContaining && ((PlayerContaining) t).getContainingPlayer() != null);
	...
	private class TileCheck implements Comparable<TileCheck> {
		 ...
		 */
		TileCheck(final TileCheck parentTile, final Vector2i tilePosition, final boolean blockedTile,
